...
4. Update your Slack app in [api.slack.com/apps](https://api.slack.com/apps) with:
    a. Commands: Add `/start` with the URL `{ngrok_url}/commands`
    b. Interactions: Use `{ngrok_url}/interactions` as the endpoint
    c. Events: Subscribe to the "message" event type and use `{ngrok_url}/events` 
    for the Events API endpoint
